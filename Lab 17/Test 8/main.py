...
# Initializing motor 
# Motor 1
ain1_ph = Pin(XXX, Pin.OUT) # Initialize GP14 as an OUTPUT
ain2_en = PWM(XXX, freq = pwm_rate, duty_u16 = 0)
# Motor 2
ain3_ph = Pin(XXX, Pin.OUT) # Initialize GP14 as an OUTPUT
ain4_en = PWM(XXX, freq = pwm_rate, duty_u16 = 0)
...
def ir_callback(data, addr, _):
    ...
    if (data == 1):
        ...
        LED3.value(0)
        ain1_ph.low()
        ain2_en.duty_u16(int(pwm))
        ain3_ph.low()
        ain4_en.duty_u16(int(pwm))
    elif (data == 2):
        print("LED 2-3: Motor off")
        LED1.value(0)
        LED2.value(1)
        LED3.value(1)
        ain1_ph.low()
